...
# Updated to 2018 values by Jakob Jakobson and Adriaan Riet, 2019
...
2019 and replaced the 2014 CODATA set. They are based on all of the data
available through 31 December 2018. The 2018 adjustment was carried out under
...
_physical_constants_2018 = parse_constants_2018(txt2018)
...
for k in _physical_constants_2018:
    if 'Boltzmann constant in inverse meters' in k:
        _aliases[k] = k.replace('meters','meter')
...
def _check_obsolete(key):
    if key in ['magn. constant','mag. constant']:
        warnings.warn("The mag. constant is no longer a true constant, \
 is now experimentally derived, and is listed as the vacuum mag. permeability",ConstantWarning)
    elif key in _obsolete_constants and key not in _aliases:
...
el_charge = value('elementary charge')
epsilon0 = 1 / (mu0 * c * c)

exact_values = {
    #'mag. constant': (mu0, 'N A^-2', 0.0), This is (as of CODATA 2018) a derived quantity
